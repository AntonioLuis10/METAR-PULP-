...
if ciudad:
    try:
        ...
        m1.metric("Viento", f"{int(round(cur['wind_speed_10m']))} kt", f"{int(cur['wind_direction_10m'])}°")
        m2.metric("Temp / Rocío", f"{round(cur['temperature_2m'])}°C", f"Td {round(cur['dew_point_2m'])}°C")
